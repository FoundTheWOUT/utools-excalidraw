...
import { storeSetItem } from "./store";
...
export class StoreSystemUtools implements StoreSystem {
  ...
  storeFile(excalidrawRef?: ExcalidrawImperativeAPI | null) {
    if (excalidrawRef && window.utools) {
      const storedFiles = utools.db.allDocs(FILE_DOC_PREFIX).map((doc) => doc._id.split("/")[1]);
      ...
      for (let fileKey in files) {
        ...
        storeFile(fileKey, encoder.encode(fileObjectStr), undefined, excalidrawRef);
      }
    }
  }
  ...
  handleLibraryChange(items: LibraryItems): void {
    ...
    const stored_lib_ids_set = new Set(libraries.map((lib: any) => lib._id.split("/")[1]));
    items.forEach((item) => {
      const { id } = item;
